				...
				if (typeof toolbar == "string") {
					...
					for (var i = 0; i < toolbar.length; i++) {
						...
						if (p == "-" ) {
                            ...
                            //给Toolbar增加按钮和事件
                            var button = $("<div class='ui-pg-div'><span class='ui-icon "+p.buttonicon+"' style='float:left'/><span style='float:left'>"+p.caption+"</span></div>").appendTo($(grid.uDiv));;
                            button[0].onclick = eval(p.onClickButton || function () { });
                            button.css("float" , "left");
                            //var icon = $("<span class='ui-icon "+p.iconCls+"'/>");
